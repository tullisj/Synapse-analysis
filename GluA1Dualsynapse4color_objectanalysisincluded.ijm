...
run("Correct 3D drift", "channel=2 correct only=0 lowest=1 highest=1 max_shift_x=120 max_shift_y=120 max_shift_z=10");
waitForUser("Select a region of dendrite then crop. press OK");
saveAs("Tiff", path+"cropped"+name);
rename("image");
...
//geph intensity
	...
	gephintensity = newArray(
		getResult("Mean",0),
		getResult("Mean",1));
	for (j=2; j<gephlength; j++) {
		gephintensity = Array.concat(gephintensity,
			getResult("Mean",j));
	}
		run("Clear Results");
...
//psd proximity to geph
	...
	g_minz = Array.deleteIndex(g_minz, 0);
	Array.show("Spine/shaft ratios",minz,camkiiPSD,glua1PSD,PSDintensity,objectarea,g_minz,camkiiGeph,glua1geph,gephintensity,gephobjectarea);
		saveAs("Results",path+"data_"+name+"_timepoint_"+i+".csv");
}
selectWindow("shaft");
saveAs("Tiff", path+"shaft"+name);
selectWindow("Results");
run("Close");
run("Close All");
